...
ahrsBringup::ahrsBringup() : rclcpp::Node("ahrs_bringup") {
  ...
  this->declare_parameter<std::string>("imu_topic", "/trunk_imu");
  ...
  //  pravite_nh.param("mag_pose_2d_topic", mag_pose_2d_topic_,
  //  std::string("/mag_pose_2d")); pravite_nh.param("Euler_angles_pub_",
  //  Euler_angles_topic_, std::string("/euler_angles"));
  //  pravite_nh.param("Magnetic_pub_", Magnetic_topic_,
  //  std::string("/magnetic"));
  ...
  gps_pub_ =
      create_publisher<sensor_msgs::msg::NavSatFix>(gps_topic.c_str(), 10);

  mag_pose_pub_ = create_publisher<geometry_msgs::msg::Pose2D>(
      mag_pose_2d_topic.c_str(), 10);

  Euler_angles_pub_ = create_publisher<geometry_msgs::msg::Vector3>(
      Euler_angles_topic.c_str(), 10);
  Magnetic_pub_ =
      create_publisher<geometry_msgs::msg::Vector3>(Magnetic_topic.c_str(), 10);

  twist_pub_ =
      create_publisher<geometry_msgs::msg::Twist>(twist_topic.c_str(), 10);
  NED_odom_pub_ =
      create_publisher<nav_msgs::msg::Odometry>(NED_odom_topic.c_str(), 10);
...
void ahrsBringup::processLoop() // 数据处理过程
{
  ...
  while (rclcpp::ok()) {
    ...
    if (if_debug_) {
      ...
      std::cout << std::endl;
      std::cout << "check_head: " << std::hex << (int)check_head[0] << std::dec
                << std::endl;
    }
    ...
    if (if_debug_) {
      std::cout << "head_type:  " << std::hex << (int)head_type[0] << std::dec
                << std::endl;
    }
    if (head_type[0] != TYPE_IMU && head_type[0] != TYPE_AHRS &&
        head_type[0] != TYPE_INSGPS && head_type[0] != TYPE_GEODETIC_POS &&
    ...
    if (head_type[0] == TYPE_IMU && check_len[0] != IMU_LEN) {
      ...
      continue;
               ...
               check_len[0] != GEODETIC_POS_LEN) {
      RCLCPP_WARN(this->get_logger(), "head_len error (GEODETIC_POS)");
      continue;
    } else if (head_type[0] == TYPE_GROUND ||
               head_type[0] == 0x50) // 未知数据，防止记录失败
    {
      uint8_t ground_sn[1];
    ...
    if (if_debug_) {
      std::cout << "check_sn: " << std::hex << (int)check_sn[0] << std::dec
                << std::endl;
      std::cout << "head_crc8: " << std::hex << (int)head_crc8[0] << std::dec
                << std::endl;
      std::cout << "head_crc16_H: " << std::hex << (int)head_crc16_H[0]
                << std::dec << std::endl;
      std::cout << "head_crc16_L: " << std::hex << (int)head_crc16_L[0]
                << std::dec << std::endl;
    }
    ...
    if (head_type[0] == TYPE_IMU) {
      ...
      uint8_t CRC8 =
          CRC8_Table(Geodetic_Position_frame_.read_buf.frame_header, 4);
    ...
    if (head_type[0] == TYPE_IMU) {
      ...
      if (if_debug_) {
        std::cout << "CRC16:        " << std::hex << (int)CRC16 << std::dec
                  << std::endl;
        std::cout << "head_crc16:   " << std::hex << (int)head_crc16 << std::dec
                  << std::endl;
        std::cout << "head_crc16_h: " << std::hex << (int)head_crc16_h
                  << std::dec << std::endl;
        std::cout << "head_crc16_l: " << std::hex << (int)head_crc16_l
                  << std::dec << std::endl;
      ...
      uint16_t head_crc16 = head_crc16_l + (head_crc16_h << 8);
      size_t data_s = serial_.read(insgps_frame_.read_buf.read_msg,
                                   (INSGPS_LEN + 1)); // 48+1
      // if (if_debug_){
      //   for (size_t i = 0; i < (AHRS_LEN + 1); i++)
      //   {
      ...
      // }
      uint16_t CRC16 =
          CRC16_Table(insgps_frame_.frame.data.data_buff, INSGPS_LEN);
      if (if_debug_) {
        std::cout << "CRC16:        " << std::hex << (int)CRC16 << std::dec
                  << std::endl;
        std::cout << "head_crc16:   " << std::hex << (int)head_crc16 << std::dec
      ...
      uint16_t head_crc16_l =
          Geodetic_Position_frame_.frame.header.header_crc16_l;
      uint16_t head_crc16_h =
          Geodetic_Position_frame_.frame.header.header_crc16_h;
      uint16_t head_crc16 = head_crc16_l + (head_crc16_h << 8);
      ...
      //   std::cout << std::dec << std::endl;
      // }
      uint16_t CRC16 = CRC16_Table(
          Geodetic_Position_frame_.frame.data.data_buff, GEODETIC_POS_LEN);
      if (if_debug_) {
        std::cout << "CRC16:        " << std::hex << (int)CRC16 << std::dec
                  << std::endl;
        std::cout << "head_crc16:   " << std::hex << (int)head_crc16 << std::dec
    ...
    if (head_type[0] == TYPE_IMU) {
      ...
      if (device_type_ == 0) //未经变换的原始数据
      {
        ...
        imu_data.angular_velocity.x =
            imu_frame_.frame.data.data_pack.gyroscope_x;
        imu_data.angular_velocity.y =
            imu_frame_.frame.data.data_pack.gyroscope_y;
        imu_data.angular_velocity.z =
            imu_frame_.frame.data.data_pack.gyroscope_z;
    ...
    //读取gps_pos数据进行解析，并发布相关话题
      ...
      // std::cout << "lat: " <<
      // Geodetic_Position_frame_.frame.data.data_pack.Latitude << std::endl;
      // std::cout << "lon: " <<
      // Geodetic_Position_frame_.frame.data.data_pack.Longitude << std::endl;
      // std::cout << "h: " <<
      // Geodetic_Position_frame_.frame.data.data_pack.Height << std::endl;
    ...
    //读取INSGPS数据进行解析，并发布相关话题
      ...
      // odom_msg.header.frame_id = odom_frame_id; // Odometer TF parent
      // coordinates //里程计TF父坐标
      odom_msg.pose.pose.position.x =
          insgps_frame_.frame.data.data_pack.Location_North; // Position //位置
      ...
      odom_msg.twist.twist.linear.x =
          insgps_frame_.frame.data.data_pack
              .Velocity_North; // Speed in the X direction //X方向速度
      odom_msg.twist.twist.linear.y =
          insgps_frame_.frame.data.data_pack
              .Velocity_East; // Speed in the Y direction //Y方向速度
      ...
      //  std::cout << "N: " <<
      //  insgps_frame_.frame.data.data_pack.Location_North << std::endl;
      //  std::cout << "E: " << insgps_frame_.frame.data.data_pack.Location_East
      //  << std::endl; std::cout << "D: " <<
      //  insgps_frame_.frame.data.data_pack.Location_Down << std::endl;
    }
  }
}

void ahrsBringup::magCalculateYaw(double roll, double pitch, double &magyaw,
                                  double magx, double magy, double magz) {
  double temp1 = magy * cos(roll) + magz * sin(roll);
  double temp2 = magx * cos(pitch) + magy * sin(pitch) * sin(roll) -
  ...
  case TYPE_INSGPS:
    if (++read_sn_ != insgps_frame_.frame.header.serial_num) {
      if (insgps_frame_.frame.header.serial_num < read_sn_) {
        sn_lost_ +=
            256 - (int)(read_sn_ - insgps_frame_.frame.header.serial_num);
        if (if_debug_) {
          RCLCPP_WARN(this->get_logger(), "detected sn lost.");
        }
      } else {
  ...
  case TYPE_GEODETIC_POS:
    if (++read_sn_ != Geodetic_Position_frame_.frame.header.serial_num) {
      if (Geodetic_Position_frame_.frame.header.serial_num < read_sn_) {
        sn_lost_ +=
            256 -
            (int)(read_sn_ - Geodetic_Position_frame_.frame.header.serial_num);
        if (if_debug_) {
          RCLCPP_WARN(this->get_logger(), "detected sn lost.");
        }
      } else {
        sn_lost_ +=
            (int)(Geodetic_Position_frame_.frame.header.serial_num - read_sn_);
        if (if_debug_) {
          RCLCPP_WARN(this->get_logger(), "detected sn lost.");
        }
      }
